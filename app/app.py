...
class App(ABC):
    def __init__(self, game, window_size: tuple[int, int]):
        ...
        self.running = True
        self.buttons_edges: list[tuple[EdgeSurface, Edge]] = []
        self.padding = (20, 30)
        self.game = game
    ...
    def handle_collision(self, pos):
        ...
        for button in self.buttons_edges:
            if button[0].hitbox_button.collidepoint(real_pos):
                if button[0].check_collision(real_pos):
                    pressed.append(button[0])
        if pressed:
